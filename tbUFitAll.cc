...
public:
    //double taut = 2100.;
    tbUFitAll(bool fix = true, double lifetime3 = tTriplet); //2.93); 3.48
    virtual ~tbUFitAll() { ; }
    TFile *inFile;
    TFile *fout;
    enum
...
void tbUFitAll::makeDataGraphs(TH1D** hList,TString tag)
{
...
void tbUFitAll::makeModelGraphs(TH1D** hList,TString tag)
{
  /*
  static double sStart = 900.;
  static double sEnd = 1020.;
...
void tbUFitAll::writeHist(TH1D *h)
{
...
void tbUFitAll::rangeIntegral(TH1D *h, double xlow, double xhigh, double& sum, double& err)
{
    sum = 0;
    err = 0;
...
TCanvas *tbUFitAll::fhistCan(TString title, TH1D *h, TF1 *fp)
{
...
TF1 *tbUFitAll::tailFit(int iset, TH1D *hTail)
{
  TF1 *ftail=NULL;
  return ftail;
}
...
TF1 *tbUFitAll::setFitD(int iset, TH1D *hLife, double ppm)
{
...
void tbUFitAll::tbFit1(int iset, TH1D *hLife)
{
    ...
    fitKA[iset] = 1./fpD[iset]->GetParameter(6);
    fitKAErr[iset] = fpD[iset]->GetParError(6)/fpD[iset]->GetParameter(6)/fpD[iset]->GetParameter(6);
    //fitKA[iset] = fpD[iset]->GetParameter(6);
    //fitKAErr[iset] = fpD[iset]->GetParError(6);
    ...
    canF->SetGridy();
    //hLife->SetTitle("");
    hLife->GetYaxis()->SetRangeUser(1.,1200.);
...
// MAIN 
tbUFitAll::tbUFitAll(bool fix, double lifetime3)
{
  ...
  for(int iset=0; iset<NSETS; ++iset) {
    ...
    fout = new TFile("tbUFitOut.root", "RECREATE");
