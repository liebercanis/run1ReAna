...
void tbTFitAll::makeDataGraphs(TH1D** hList,TString tag)
{
  ...
  dSinglet = new TGraphErrors(NSETS-1, fitPPM, dSingletIntegral,fitPPMErr, dSingletIntErr);
  ...
  dSinglet->SetMarkerSize(.4);

  dTriplet = new TGraphErrors(NSETS-1,fitPPM, dTripletIntegral,fitPPMErr, dTripletIntErr);
  ...
  dTriplet->SetMarkerSize(.4);

  dTotTriplet = new TGraphErrors(NSETS-1, fitPPM, dTripletTotalIntegral,fitPPMErr, dTripletTotalIntErr );
  ...
  dTotTriplet->SetMarkerSize(.4);

  dTot = new TGraphErrors(NSETS-1, fitPPM, dTotalIntegral,fitPPMErr,dTotalIntErr );
  dTot->SetName(Form("RangeTotalModel%s",tag.Data()));
  dTot->SetTitle(Form("data total range integral %0.2f - %0.2f ", sStart, tEnd));
  dTot->SetMarkerColor(kBlue);
  dTot->SetMarkerStyle(21);
  dTot->SetMarkerSize(.4);

 
  fout->Append(dSinglet);
  fout->Append(dTriplet);
  fout->Append(dTotTriplet);
  fout->Append(dTot);
...
void tbTFitAll::makeModelGraphs(TH1D** hList,TString tag)
{
  ...
  */
  ...
  gSinglet = new TGraphErrors(NSETS-1, fitPPM, mSingletIntegral,fitPPMErr, mSingletIntErr);
  ...
  gTriplet = new TGraphErrors(NSETS-1,fitPPM, mTripletIntegral,fitPPMErr, mTripletIntErr);
  ...
  gTotTriplet = new TGraphErrors(NSETS-1, fitPPM, mTripletTotalIntegral,fitPPMErr, mTripletTotalIntErr );
  ...
  gTot = new TGraphErrors(NSETS-1, fitPPM, mTotalIntegral,fitPPMErr,mTotalIntErr );
  gTot->SetName(Form("RangeTotalModel%s",tag.Data()));
  ...
  printf(" makeModelGraphs Singlet  %s \n",tag.Data());
  gSinglet->Print("a");
  printf(" makeModelGraphs Triplet  %s \n",tag.Data());
  gTriplet->Print("a");
  printf(" makeModelGraphs TotTriplet  %s \n",tag.Data());
  gTotTriplet->Print("a");
...
*/
TF1 *tbTFitAll::setFit(int iset, TH1D *hLife, double ppm)
{
...
void tbTFitAll::tbFit1(int iset, TH1D *hLife)
{
    ...
    fpD[iset] = setFit(iset, hLife, runPPM[iset]);
    TH1D *hLifeD = (TH1D *)hLife->Clone(Form("LifeDfit%i", iset));
    fpD[iset] = setFitD(iset, hLifeD, runPPM[iset]);
    ...
    hFitDHist[iset] = (TH1D*) fpD[iset]->GetHistogram();
    hFitDHist[iset]->SetName(Form("FitDHist-%i",iset));
    ...
    vk1All.push_back(fpD[iset]->GetParameter(4)/kplusZero);
    vk1AllErr.push_back(fpD[iset]->GetParError(4)/kplusZero);
...
tbTFitAll::tbTFitAll(bool fix, double lifetime3)
{
  ...
  for(int iset=0; iset<NSETS; ++iset) {
    ...
    outTag=TString("float");
    makeModelGraphs(hFitHist,outTag);
    //makeModelGraphs(hFitDHist,"segreto");
    ...
    TGraphErrors *gtau3Mu = new TGraphErrors(1, &vset[NSETS-1], &vtautAll[NSETS-1], &vsetErr[NSETS-1], &vtautAllErr[NSETS-1]);
