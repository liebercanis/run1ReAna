...
void eventAna()
{
  ...
  for(int i=0; i<NSETS; ++i) {
    hWave[i]=NULL;
    inFile->GetObject(Form("Wave-%i",i),hWave[i]);
    if(hWave[i]) {
      cout << " wave hist " << hWave[i]->GetName();
      fout->Append(hWave[i]);
    }
  }
  ...
  for(int iset=0; iset<NSETS; ++iset ) printf(" set %i runs %i total events %i \n ", iset, setRuns[iset], setTotal[iset]);
  ...
  for(int iv=0; iv< NPRE; ++iv ) tPre->SetBranchAddress(preNames[iv],&preVars[iv]);
  ...
  for (Long64_t jent=0;jent<tPre->GetEntries() ;jent ++) {
    ...
    if(flag==0) prePass[iset] = prePass[iset] + 1.;
  }
  ...
  vector<double> vset;
  for(unsigned iset =0; iset<preFrac.size(); ++iset) vset.push_back(double(iset));
  ...
  for (Long64_t jent=0;jent<tEvent->GetEntries() ;jent ++) {
    ...
    if(run<0) run = evVars[EVRUN] - 3000;
    if(run!=thisRun) {
      if(thisRun>0) {
  ...
  hSingleSet[0]->Draw("");
  for(int iset=1; iset < NSETS; ++iset) hSingleSet[iset]->Draw("sames");
  ...
  hTripleSet[0]->Draw("");
  for(int iset=1; iset < NSETS; ++iset) hTripleSet[iset]->Draw("sames");
  ...
  for(int i=0; i<NSETS; ++i) {
    if(hWave[i]) {
      hWave[i]->Scale(1./double(ngood[i]));
      cout << " wave hist " << hWave[i]->GetName() << " nev " << ngood[i] ;
      cout << " singlet " << hWave[i]->Integral( hWave[i]->FindBin(singletStart), hWave[i]->FindBin(singletEnd)  ) ;
      cout << " triplet " << hWave[i]->Integral( hWave[i]->FindBin(singletEnd), hWave[i]->FindBin(100000)  ) << endl;
    }
  }
