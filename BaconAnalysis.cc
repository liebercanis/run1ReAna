...
void BaconAnalysis(int maxFiles ){
  ...
  TNtuple *ntSummary = new TNtuple("Summary", " summary",sumString.Data());
  TNtuple *ntEvPre = new TNtuple("EvPre", " event ", preString.Data());
  TNtuple *ntEvent = new TNtuple("Event", " event ", evString.Data());
  ...
  for (int i = 0; i < int(files.size()); i++)
  {
    ...
    */
    ...
    for (int j = 0; j < nentries; j++)
    {
      ...
      if(j%1000==0)
        cout << " first loop event  " << j << " v1 entries " << h_v1->GetEntries() << endl;
      for(int k = 0;k<m;k++){
        h_v1->SetBinContent(k, h_v1->GetBinContent(k) + oneEvent->volt1.at(k));
      ...
      if (j % 1000 == 0)
      {
        ...
        printf("  \t\t total %i good %i v1 %E \n",
               ntotal, nrungood, h_v1->GetEntries() / double(h_v1->GetNbinsX()));
      }
      ...
      // fill evvars
      preVars[PREEVENT] = float(j);
      preVars[PRERUN] = filenumber;
      preVars[PRESET] = runSet;
      preVars[PREFLAG] = event_code;
      preVars[PRESUM] = getSum(h_v1, singletStart, double(nDigi));
      preVars[PRESINGLET] = getSum(h_v1, singletStart , singletEnd);
      preVars[PRETRIPLET] = getSum(h_v1, singletEnd , double(nDigi));
      preVars[PRELATE] = vmaxPre;
      preVars[PRELATETIME] = maxTimePre;
      ntEvPre->Fill(preVars);
      ++nruntotal;
      if (event_code > 0)
        continue;
      ...
      //second loop
      if (nrungood % 500 == 0) {
        cout << " event passes  event   " << j << " set  " << currentSet << endl;
        printf("  \t\t total %i good %i v1b %E \n", 
          ntotal, nrungood, h_v1b->GetEntries()/double(h_v1b->GetNbinsX()));
      }
      for (int k = 0; k < m; k++)
      {
        double l = -1 * (oneEvent->volt1.at(k)  - baseline_mean);

        h_v1b->SetBinContent(k, h_v1b->GetBinContent(k)+ l);
        h_v1c->SetBinContent(k, h_v1c->GetBinContent(k) + l / wf_min);
      ...
      if (wf_singlet <= 15)
        ...
        if ((nrungood % 100 == 0))
        {
          ...
          cout << " -- entry " << j << " file " << filenumber << " ngood " << nrungood << " intgral v1c  " << h_v1c->Integral(900, 1100) << endl;
    ...
    //  << " sec events " << ntotal << " good " 
    //  << nrungood << endl;
    int time_seconds = int(chrono::duration_cast<chrono::seconds>(end - start).count());
    printf("  ENDOFFILE %i  sec %i total %i good %i v1b %E \n", filenumber,time_seconds,
           ntotal, nrungood, h_v1b->GetEntries() / double(h_v1b->GetNbinsX()));
